...
@section('content')
    ...
    <div class="">
                        ...
                        @foreach($categories as $category)
                            <tr class="border border-e hover:bg-gray-50 text-neutral-500">
                                ...
                                <td class="py-1 text-center border" colspan="2">
                                    <button type="submit" form="form-edit" formaction="{{ route('categories.edit', ['category' => $category->id]) }}" title="editar">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                        </svG>
                                    </button>
                                    <button type="submit" form="form-remove" formaction="{{ route('categories.destroy', ['category' => $category->id]) }}" title="remover">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
        ...
        <a href="{{ route('categories.create') }}" class="bg-green-600 rounded-md py-2 px-3 text-neutral-100 shadow-md shadow-neutral-400" title="Cadastrar Categoria">Cadastrar Categoria</a>
        <div>
            @if($categories)
                {{ $categories->links() }}
            @endif
